...
class tinyshell {
    ...
    move = function (ev) {
        ...
        if (this.Events['revealleft'].detail.callback || this.Events['revealright'].detail.callback) {
            ...
            if (touches.length == 1) {
                ...
                //console.log('dir', dir);
                if ((this.Events['revealleft'].detail.callback && dir === 'left') ||
                    (this.Events['revealright'].detail.callback && dir === 'right')) {
                    //need to handle drag with or without reveal areas
                    let move = 0;
                    if (dir === 'right' && (this.element.classList.contains('has-reveal-left') || (this.element.classList.contains('has-reveal-both')))) {
                        move = Math.min((deltaX + (-1 * this.Params.twenty)), 0);
                    } else {
                        //nothing to reveal
                        move = 0;
                    }
                    if (dir === 'left' && (this.element.classList.contains('has-reveal-right')) || (this.element.classList.contains('has-reveal-both'))) {
                        if (this.element.classList.contains('has-reveal-left') || this.element.classList.contains('has-reveal-both')) {
                            //starting at -20vw
                            move = (-1 * Math.min(this.Params.twenty, deltaX)) - this.Params.twenty;
    ...
    start = function (ev) {
        ...
        if (touches.length == 1) {
            ...
            //for revealleft
            if (this.Events['revealleft'].detail.callback &&
                dir == 'left' &&
                typeof this.Events['swiperight'].detail.callback === 'function') {
                ...
                if (Math.abs(deltaX) < this.Params.minDrag) {
                    ...
                    if (this.element.classList.contains('has-reveal-left') || (this.element.classList.contains('has-reveal-both'))) {
                        this.element.style.transform = `translateX(${this.Params.twenty * -2}px)`;
                    } else {
                        this.element.style.transform = `translateX(${this.Params.twenty * -1}px)`;
                    }
                } else {
                    //between min and max...
                    if (this.element.classList.contains('has-reveal-left') || this.element.classList.contains('has-reveal-both')) {
            ...
            if (deltaX < this.Params.minDrag && (this.Events['revealright'].detail.callback || this.Events['revealleft'].detail.callback)) {
